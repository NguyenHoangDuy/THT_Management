...
$(document).ready(function () {
    ...
    //active menu
    resetMenu();
    activeMenu(1, 2, 2, '2_3', 'menu_Utilities_Parameters');
    ...
    $("#formPopupRe").validate({
        ...
        messages: {
            ParamID: {
                ...
                maxlength: "Chỉ nhập 5 ký tự: Gồn 2 ký tự đầu là chữ + 3 ký tự số",
            },
            Type: {
                required: "Thông tin bắt buộc",
            },
...
function onOpenPopupDe(key, obj) {
    ...
    if (key == 0) {     // Create
        ...
        $.post(r + "/Utilities_Parameters/GetReasonyCode", { ReasonID: id }, function (data) {
            showLoading();
            if (data.success) {
                var value = data.data;
...
function DeleteItem() {
    ...
    if (listrowid != null && listrowid != "") {
        ...
        if (c == true) {
            $.post(r + "/Utilities_Parameters/DeleteItem", { data: listrowid }, function (data) {
                if (data.success) {
